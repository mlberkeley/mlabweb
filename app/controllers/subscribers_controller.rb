class SubscribersController < ApplicationController
  ...
  before_action :authenticate_member!, only: [:index, :edit, :update, :destroy]
  before_action :exec_member, only: [:index, :edit, :update, :destroy]
  ...
  def update
    respond_to do |format|
      if @subscriber.update(subscriber_params)
        format.html { redirect_to @subscriber, notice: 'Preferences successfully updated.' }
  ...
  private
    ...
    def correct_member
      # @subscriber = Subscriber.find(params[:id])
      # redirect_to(@subscriber) unless (#current subscriber or member_signed_in?)
    end
end
